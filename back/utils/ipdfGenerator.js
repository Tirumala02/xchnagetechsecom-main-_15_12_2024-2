...
async function fetchHSNFromPython(query) {
    ...
    try {
        const response = await axios.post('https://xchange-hsn-code.onrender.com', {
        ...
        try {
            ...
            for (const item of order.items) {
                ...
                tableRows.push([
                    ...
                    `${(item.quantity * (item.price_upper || item.iprice))}`,
                ]);
            }
            ...
            const table = {
                title: 'Order Details',
                headers: ['Item Name', 'HSN Code', 'Quantity', 'Unit Price', 'Amount', "Amount with GST"],
                rows: tableRows,
            };
            ...
            const itotalAmount = order.items.reduce((sum, item) => sum + item.quantity * item.iprice, 0) + (order.deliveryCharge || 0);
            const difference = itotalAmount - totalAmount;
            ...
            doc.text(`GST       : ${difference.toFixed(2)}/-`);
            doc.text(`Total     : ${itotalAmount.toFixed(2)}/-`);
